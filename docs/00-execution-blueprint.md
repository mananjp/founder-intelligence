...
The Bible (47 sections) is the *what* and *why*. This document is the *how*: who builds it, in what
...
The Bible's system naturally splits into five *layers* (data/schema, AI research engine, decision
...
**Migration path is intentionally boring:** almost nothing about the *code* changes between MVP and
Production — same Express app, same FastAPI app, same schema. What changes is *where it runs* and
*how it scales*. That's why the repo already has both `infra/render.yaml` (MVP) and
...
`claim_contradictions`, so the UI's Evidence screen and Decision Trace can render the *why* behind
...
- **Model routing**: every LLM call goes through `llm/router.py` by *task name* (`extract_evidence`,
...
- **CODEOWNERS** (`.github/CODEOWNERS`) map paths → *squads*, not individuals, so reshuffling people
...
This split is deliberate: the scaffold encodes every *structural* decision from the Bible (schema,
