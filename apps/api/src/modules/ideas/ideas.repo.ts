...
export const ideasRepo = {
  list: (workspaceId: string) =>
    withWorkspace(workspaceId, async (c) =>
      (await c.query("SELECT * FROM ideas WHERE workspace_id=$1 ORDER BY created_at DESC", [workspaceId])).rows),
  get: (workspaceId: string, id: string) =>
    withWorkspace(workspaceId, async (c) =>
      (await c.query("SELECT * FROM ideas WHERE workspace_id=$1 AND id=$2", [workspaceId, id])).rows[0] ?? null),
