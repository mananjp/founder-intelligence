...
researchRouter.post("/runs", validate({ body: startBody }), async (req, res) => {
  const { ideaId } = req.params as { ideaId: string };
  const run = await withWorkspace(req.workspaceId!, async (c) => {
    const brief = await c.query("SELECT id FROM research_briefs WHERE idea_id=$1 ORDER BY version DESC LIMIT 1", [ideaId]);
    if (!brief.rows[0]) throw new HttpError(409, "brief_missing", "Complete the adaptive interview first");
    return (await c.query(
      ...
      [req.workspaceId, ideaId, brief.rows[0].id, req.body.depth, req.body.geography, req.user!.id],
    )).rows[0];
  });
